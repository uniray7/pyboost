...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::projectPoints

void cv::widefov::projectPoints(InputArray objectPoints, OutputArray imagePoints, const Affine3d& affine,
...
void cv::widefov::projectPoints(InputArray objectPoints, OutputArray imagePoints, InputArray _rvec,
        InputArray _tvec, InputArray _K, InputArray _D, double alpha, OutputArray jacobian)
{
    ...
    for(size_t i = 0; i < n; ++i)
    {
        ...
        double theta;
        double r, r2;
        // deal with 0 case, TODO: floating-point equality compare is BAD
        if(Y[2] == 0.0){
            x = Vec2d(Y[0], Y[1]);
            r2 = x.dot(x);
            r = std::sqrt(r2);
            theta = CV_PI/2;
             ...
             x = Vec2d(Y[0]/Y[2], Y[1]/Y[2]);
             r2 = x.dot(x);
             r = std::sqrt(r2);
             // Angle of the incoming ray:
             theta = atan(r);
             // allow 180 degree up projection
...
#ifdef AMSD_IRON_WIDE_FOV_FIX
            if(Y[2] < 0.0) // allow 180 degree up projection
                Y[2] = -Y[2];
#endif
            //Vec2d x(Y[0]/Y[2], Y[1]/Y[2]);
            Vec3d dxdom[2];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::distortPoints

void cv::widefov::distortPoints(InputArray undistorted, OutputArray distorted, InputArray K, InputArray D, double alpha)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::undistortPoints

void cv::widefov::undistortPoints( InputArray distorted, OutputArray undistorted, InputArray K, InputArray D, InputArray R, InputArray P)
{
    // will support only 2-channel data now for points
...
#else
    double alpha;// consider skew factor
    if (K.depth() == CV_32F)
    {
        Matx33f camMat = K.getMat();
        f = Vec2f(camMat(0, 0), camMat(1, 1));
...
#ifdef AMSD_IRON_WIDE_FOV_FIX
        pw[0] -= alpha * pw[1]; // consider skew factor
#endif
        double scale = 1.0;
...
#else
        // reproject: handle over 180 degree projection
        Vec2d pu;
        Vec3d pr;
        Vec2d fi;
        if(theta_d == 0.0){
            ...
            pu = pw * scale;
            pr = RR * Vec3d(pu[0], pu[1], 1.0);
            fi = Vec2d(pr[0]/pr[2], pr[1]/pr[2]);
        }
        else{
            pu = pw * scale;
            pu[0] = -pu[0];
...
void cv::widefov::initUndistortRectifyMap( InputArray K, InputArray D, InputArray R, InputArray P,
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::undistortImage

void cv::widefov::undistortImage(InputArray distorted, OutputArray undistorted,
        InputArray K, InputArray D, InputArray Knew, const Size& new_size)
{
    Size size = new_size.area() != 0 ? new_size : distorted.size();

    cv::Mat map1, map2;
    widefov::initUndistortRectifyMap(K, D, cv::Matx33d::eye(), Knew, size, CV_16SC2, map1, map2 );
    cv::remap(distorted, undistorted, map1, map2, INTER_LINEAR, BORDER_CONSTANT);
}


//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::estimateNewCameraMatrixForUndistortRectify

void cv::widefov::estimateNewCameraMatrixForUndistortRectify(InputArray K, InputArray D, const Size &image_size, InputArray R,
...
#ifdef AMSD_IRON_WIDE_FOV_FIX
    // compute new principal point by reprojecting principal point of fisheye image to undistorted image
    cv::Mat cp(1,1,CV_64FC2);
    cp.at<Vec2d>(0)[0] = K.getMat().at<double>(0,2);
    cp.at<Vec2d>(0)[1] = K.getMat().at<double>(1,2);
    fisheye::undistortPoints(cp, cp, K, D, R);
    Vec2d cn2 = cp.at<Vec2d>(0);
    new_c = -cn2 * f + Vec2d(w, h * aspect_ratio) * 0.5;
#endif
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::stereoRectify

void cv::widefov::stereoRectify( InputArray K1, InputArray D1, InputArray K2, InputArray D2, const Size& imageSize,
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::calibrate

double cv::widefov::calibrate(InputArrayOfArrays objectPoints, InputArrayOfArrays imagePoints, const Size& image_size,
                                    ...
                                    int flags , cv::TermCriteria criteria)
{
    ...
    using namespace widefov::internal;
    //-------------------------------Initialization
    ...
    std::vector<Vec3d> curr_omc(objectPoints.total()), curr_Tc(objectPoints.total());

    CalibrateExtrinsics(objectPoints, imagePoints, finalParam, check_cond, thresh_cond, omc, Tc);
    curr_omc = omc;
    curr_Tc = Tc; // AMSD_IRON_WIDE_FOV_FIX
    ...
    for(int iter = 0; ; ++iter)
    {
        ...
        Mat JJ2_inv, ex3, JJ2;
        ComputeJacobians(objectPoints, imagePoints, finalParam, omc, Tc, check_cond,thresh_cond, JJ2_inv, ex3, JJ2);
        ...
        // === AMSD_IRON_WIDE_FOV_FIX ===
        // check projected points based on current parameters
        // change omc and Tc also
        for(size_t image_idx = 0; image_idx < imagePoints.total(); ++image_idx){
            int offset = 9 + 6*image_idx;
            curr_omc[image_idx][0] += G.at<double>(offset, 0);
            curr_omc[image_idx][1] += G.at<double>(offset+1, 0);
            curr_omc[image_idx][2] += G.at<double>(offset+2, 0);
            curr_Tc[image_idx][0] += G.at<double>(offset+3, 0);
            curr_Tc[image_idx][1] += G.at<double>(offset+4, 0);
            curr_Tc[image_idx][2] += G.at<double>(offset+5, 0);
            //cout << G.at<double>(offset, 0)<<","<<G.at<double>(offset+1,0)<<","<<G.at<double>(offset+2, 0)<<endl;
        }
        finalParam = currentParam;
        omc = curr_omc;
        Tc = curr_Tc; // AMSD_IRON_WIDE_FOV_FIX
    ...
    //-------------------------------Validation
    double rms;
    internal::EstimateUncertainties(objectPoints, imagePoints, finalParam,  omc, Tc, errors, err_std, thresh_cond,
                              check_cond, rms);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// cv::widefov::stereoCalibrate

double cv::widefov::stereoCalibrate(InputArrayOfArrays objectPoints, InputArrayOfArrays imagePoints1, InputArrayOfArrays imagePoints2,
...
void cv::widefov::internal::subMatrix(const Mat& src, Mat& dst, const vector<int>& cols, const vector<int>& rows)
{
...
cv::widefov::internal::IntrinsicParams::IntrinsicParams():
    f(Vec2d::all(0)), c(Vec2d::all(0)), k(Vec4d::all(0)), alpha(0), isEstimate(9,0)
{
}

cv::widefov::internal::IntrinsicParams::IntrinsicParams(Vec2d _f, Vec2d _c, Vec4d _k, double _alpha):
    f(_f), c(_c), k(_k), alpha(_alpha), isEstimate(9,0)
{
}

cv::widefov::internal::IntrinsicParams cv::widefov::internal::IntrinsicParams::operator+(const Mat& a)
{
    ...
    tmp.alpha   = this->alpha   + (isEstimate[4] ? ptr[j++] : 0); // we consider skew factor, so different from opencv to add this var
#else
...
void cv::widefov::internal::IntrinsicParams::Init(const cv::Vec2d& _f, const cv::Vec2d& _c, const cv::Vec4d& _k, const double& _alpha)
{
    this->c = _c;
    this->f = _f;
    this->k = _k;
...
void cv::widefov::internal::projectPoints(cv::InputArray objectPoints, cv::OutputArray imagePoints,
                   ...
                   const IntrinsicParams& param, cv::OutputArray jacobian)
{
    ...
    Matx33d K(param.f[0], param.f[0] * param.alpha, param.c[0],
                       ...
                       0,                        0,         1);
    widefov::projectPoints(objectPoints, imagePoints, _rvec, _tvec, K, param.k, param.alpha, jacobian);
}

void cv::widefov::internal::ComputeExtrinsicRefine(const Mat& imagePoints, const Mat& objectPoints, Mat& rvec,
...
cv::Mat cv::widefov::internal::ComputeHomography(Mat m, Mat M)
{
    int Np = m.cols;
...
cv::Mat cv::widefov::internal::NormalizePixels(const Mat& imagePoints, const IntrinsicParams& param)
{
    CV_Assert(!imagePoints.empty() && imagePoints.type() == CV_64FC2);
...
void cv::widefov::internal::InitExtrinsics(const Mat& _imagePoints, const Mat& _objectPoints, const IntrinsicParams& param, Mat& omckk, Mat& Tckk)
{
    ...
    Mat H = internal::ComputeHomography(imagePointsNormalized, X_new(Rect(0,0,X_new.cols,2)));
...
void cv::widefov::internal::CalibrateExtrinsics(InputArrayOfArrays objectPoints, InputArrayOfArrays imagePoints,
...
void cv::widefov::internal::ComputeJacobians(InputArrayOfArrays objectPoints, InputArrayOfArrays imagePoints,
                      ...
                      const int& check_cond, const double& thresh_cond, Mat& JJ2_inv, Mat& ex3, Mat& JJ3)
{
    ...
    // 9 intrinsic parameters + 6 * #img extrinsic parameters(chessboards' rotation and translation)
    // Mat JJ3 = Mat::zeros(9 + 6 * n, 9 + 6 * n, CV_64FC1);
    JJ3 = Mat::zeros(9 + 6 * n, 9 + 6 * n, CV_64FC1);
    ...
    internal::subMatrix(JJ3, JJ3, idxs, idxs);
    internal::subMatrix(ex3, ex3, std::vector<int>(1, 1), idxs);
    JJ2_inv = JJ3.inv();
}

void cv::widefov::internal::EstimateUncertainties(InputArrayOfArrays objectPoints, InputArrayOfArrays imagePoints,
                           ...
                           IntrinsicParams& errors, Vec2d& std_err, double thresh_cond, int check_cond, double& rms)
{
    ...
    Mat _JJ2_inv, ex3, JJ2;
    ComputeJacobians(objectPoints, imagePoints, params, omc, Tc, check_cond, thresh_cond, _JJ2_inv, ex3, JJ2);
...
void cv::widefov::internal::dAB(InputArray A, InputArray B, OutputArray dABdA, OutputArray dABdB)
{
...
void cv::widefov::internal::JRodriguesMatlab(const Mat& src, Mat& dst)
{
...
void cv::widefov::internal::compose_motion(InputArray _om1, InputArray _T1, InputArray _om2, InputArray _T2,
...
double cv::widefov::internal::median(const Mat& row)
{
...
cv::Vec3d cv::widefov::internal::median3d(InputArray m)
{
...
void cv::widefov::projectPoint(InputArray _objectPoint, OutputArray _imagePoint, InputArray _K, InputArray _D, double alpha)
{
    cv::Mat objectPoint = _objectPoint.getMat();
    cv::Mat K = _K.getMat();
    cv::Mat D = _D.getMat();
    ...
    if (K.depth() == CV_32F)
    {
        ...
        f = Vec2f(K.at<float>(0, 0), K.at<float>(1, 1));
        c = Vec2f(K.at<float>(0, 2), K.at<float>(1, 2));
    }
    else
    {

        f = Vec2d(K.at<double>(0, 0), K.at<double>(1, 1));
        c = Vec2d(K.at<double>(0, 2), K.at<double>(1, 2));
    }
        Vec4d k = D.depth() == CV_32F ? (Vec4d) D.at<Vec4f>(0): D.at<Vec4d>(0);
        Vec3f Xf = objectPoint.at<Vec3f>(0);
        Vec3d Xd = objectPoint.at<Vec3d>(0);
        Vec3d Xi = objectPoint.depth() == CV_32F ? (Vec3d)Xf : Xd;
        Vec3d Y = Xi;
        ...
        double theta;
        double r;
        // add by iron: to deal with 0 case
        if(Y[2] == 0.0){
            x = Vec2d(Y[0], Y[1]);
            double r2 = x.dot(x);
            r = std::sqrt(r2);
            theta = CV_PI/2;
             ...
             // Angle of the incoming ray:
             theta = atan(r);
             // add by iron: allow 180 degree up projection
            if(Y[2] < 0.0){
                theta = CV_PI - theta;
                x = -x;
            }
        ...
        _imagePoint.create(1,1,CV_64FC2);
        cv::Mat imagePoint = _imagePoint.getMat();

        imagePoint.at<Vec2d>(0) = final_point;
}
